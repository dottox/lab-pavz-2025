...
TEST_CASE("FacturaLocal: Datos inválidos")
{
    DtFecha fecha(1, 1, 2023);
    DtHora hora(12, 0, 0);
    OrderedDictionary *productos = new OrderedDictionary();
    ...
    CHECK_THROWS_AS({
        DtFactura factura(0, 0, fecha, hora, productos, -100.0f, -10, -90.0f, -18.0f);
        DtFacturaLocal dto(factura, "Mozo Juan"); }, std::invalid_argument);

    delete productos;
}

TEST_CASE("FacturaLocal: Datos nulos")
{
    DtFecha fecha(1, 1, 2023);
    DtHora hora(12, 0, 0);
    OrderedDictionary *productos = new OrderedDictionary();

    CHECK_THROWS_AS(DtFactura factura(1, 0, fecha, hora, nullptr, 100.0f, 10, 90.0f, 18.0f), std::invalid_argument);

    delete productos;
}

TEST_CASE("FacturaLocal: Productos con datos inválidos")
{
    DtFecha fecha(1, 1, 2023);
    DtHora hora(12, 0, 0);
    OrderedDictionary *productos = new OrderedDictionary();
...
TEST_CASE("FacturaLocal: Transporte inválido en DtFacturaDomicilio")
{
    DtFecha fecha(1, 1, 2023);
    DtHora hora(12, 0, 0);
    OrderedDictionary *productos = new OrderedDictionary();
    ...
    CHECK_THROWS_AS({
        DtFacturaDomicilio facturaDomicilio(1, fecha, hora, productos, 100.0f, 10, 90.0f, 18.0f, "Repartidor Juan", (Transporte)999);
